<template>
  <div class="bg-white p-6 rounded-lg shadow-sm h-full flex flex-col">
    <div class="flex justify-between items-center mb-4">
      ...
      <div>
        <el-button v-permission="'sys:menu:add'" type="primary" @click="handleAdd">新增资源</el-button>
    ...
    >
      <el-table-column prop="name" label="菜单/按钮名称" show-overflow-tooltip width="220">
      ...
      <el-table-column label="操作" width="200">
        <template #default="{ row }">
          <el-button v-permission="'sys:menu:add'" link type="primary" @click="handleAddSubMenu(row)">新增</el-button>
          <el-button v-permission="'sys:menu:update'" link type="primary" @click="handleEdit(row)">编辑</el-button>
...
const menuNodesOnly = computed(() => {
  const filterMenus = (list: any[]) => {
    return list.filter(m => m.typeFlag !== 'BUTTON').map(m => {
      if (m.children) {
        return { ...m, children: filterMenus(m.children) }
      }
      return m
    })
  }
